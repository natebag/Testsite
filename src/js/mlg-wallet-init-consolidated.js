...
class MLGWalletInitializer {
  ...
  async _doInitialize(options = {}) {
    ...
    try {
      ...
      const { getWalletManager, initializeWallet } = await import('../wallet/phantom-wallet.js');
      
      // Import voting integration if needed
      let getVotingIntegration, initializeVotingIntegration;
      if (options.enableVoting !== false) {
        try {
          const votingModule = await import('./mlg-voting-integration.js');
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    initializeMLGWallet: window.initializeMLGWallet,
    connectMLGWallet: window.connectMLGWallet,
    disconnectMLGWallet: window.disconnectMLGWallet,
    getMLGWalletInfo: window.getMLGWalletInfo,
    isMLGWalletConnected: window.isMLGWalletConnected
  };
}
